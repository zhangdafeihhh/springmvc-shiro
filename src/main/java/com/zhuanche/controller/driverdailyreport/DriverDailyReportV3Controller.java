...
import com.zhuanche.serv.driverdailyreport.DriverDailyReportV3Service;
 ...
 */
@Controller
@RequestMapping(value = "/driverDailyReportV3")
public class DriverDailyReportV3Controller extends DriverQueryController {

	private static Logger log =  LoggerFactory.getLogger(DriverDailyReportV3Controller.class);

	@Autowired
	private DriverDailyReportV3Service driverDailyReportV3Service;
	...
	public AjaxResponse queryDriverWeekReportDataNew(String licensePlates, String driverName, String driverIds, String teamIds,
		...
		if ( reportType==0 ) {
			pages = driverDailyReportV3Service.findDayDriverDailyReportByparam(params);
		}else{
			pages = driverDailyReportV3Service.findWeekDriverDailyReportByparam(params,  statDateStart,    statDateEnd);
		}

		//如果不为空，进行查询供应商名称
		List<DriverDailyReportDTO> dtoList = driverDailyReportV3Service.selectSuppierNameAndCityNameDays(pages.getList(),reportType,statDateStart,statDateEnd);
	...
	public AjaxResponse queryDriverWeekReportDataNew(@Verify(rule = "required",param = "statDateStart") String driverIds,
		...
		try {
			list = this.driverDailyReportV3Service.queryDriverReportData(params);
			total = (int) p.getTotal();
		} finally {
			PageHelper.clearPage();
		}
		//如果不为空，进行查询供应商名称
		List<DriverDailyReportDTO> dtoList = driverDailyReportV3Service.selectSuppierNameAndCityNameDays(list,0,statDateStart,statDateEnd);
...
//						return AjaxResponse.success("没有查到符合条件的数据");
			...
			if(!(StringUtils.isNotEmpty(params.getGroupIds()) && (StringUtils.isEmpty(driverList)))){
				...
				if ( reportType==0 ) {
					...
					pageInfos = driverDailyReportV3Service.findDayDriverDailyReportByparam(params);
...
//						return AjaxResponse.success("没有查到符合条件的数据");
					...
					rows = driverDailyReportV3Service.selectSuppierNameAndCityNameDays(result,reportType,statDateStart,statDateEnd);
					...
					pageInfos = driverDailyReportV3Service.findWeekDriverDailyReportByparam(params,  statDateStart,    statDateEnd);
					int pages = pageInfos.getPages();//临时计算总页数
	...
	public DriverDailyReportParams chuliDriverDailyReportEntity(DriverDailyReportParams driverDailyReportBean){
		...
		if(StringUtils.isNotEmpty(driverDailyReportBean.getStatDateStart())){
			driverDailyReportBean.setTableName("driver_daily_report_" + driverDailyReportBean.getStatDateStart().substring(0, 7).replace("-", "_"));
		}
